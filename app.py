...
from utils.utils import adaptive_instance_normalization, get_mean_std
...
decoder_path = torch.load("./experiment/trial/decoder_final.pth", map_location=device)
decoder.load_state_dict(decoder_path) #torch.load('./experiment/trial/decoder_4.pth')
...
def style_transfer(content_img, style_img, encoder, decoder, alpha, device):
    ...
    content_image = content_transform(content_img).unsqueeze(0).to(device) #apply transforms and adding batch size
    style_image = content_transform(style_img).unsqueeze(0).to(device)
...
def save_image(image, path):
    ...
    image = transforms.ToPILImage()(image) # tensor_image -> image
    image.save(path)
...
def index():
    ...
    if form.validate_on_submit():
        ...
        if content_filename and style_filename:
            print("content_filename =", repr(content_filename))
            print("style_filename   =", repr(style_filename))
            content_path = os.path.join(app.config['UPLOAD_FOLDER'], content_filename)
            style_path = os.path.join(app.config['UPLOAD_FOLDER'], style_filename)
            print("content_path     =", repr(content_path))
            print("style_path       =", repr(style_path))
